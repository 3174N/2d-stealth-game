    ...
    public GameObject[] Generate()
    {
                ...
                if (hits[x, y])
                {
                    ...
                    else
                    {
                        ...
                        currentInstance.transform.localScale =
                            new Vector3(currentInstance.transform.localScale.x + 1.0f, 1.0f, 0.0f);
        ...
        for (int y = 0; y < countY - 1; y++)
        {
            ...
            for (int x = 0; x < countX; x++)
            {
                ...
                {
                    if (bottomInstance != topInstance &&
                        bottomInstance.transform.localScale.x == topInstance.transform.localScale.x)
                    {
                        //merge! enlarge bottom instance...

                        bottomInstance.transform.localScale = new Vector3(bottomInstance.transform.localScale.x,
                            bottomInstance.transform.localScale.y + 1.0f, 0.0f);
...
public class GridShadowCastersGeneratorEditor : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("Generate"))
        {
            ...
            Undo.RecordObject(generator.shadowCastersContainer,
                "GridShadowCastersGenerator.generate"); // this does not work :(
